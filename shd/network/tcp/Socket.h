...
#define RETURN_IF_SOCKET_INVALID_(VALUE)  if (!isValid()) { LOG_ERROR("Socket is NOT initialized : " << __FILE__ << " " << __LINE__); return VALUE; }
    ...
    bool write(const char* iByteArray, const size_t iSize) {
      ...
      LOG_MESSAGE("[Socket] Send : " << static_cast<int>(((shd::network::message::Header*)iByteArray)->code) << " -- Payload size : " << ((shd::network::message::Header*)iByteArray)->payloadSize);
      int iResult = send(_socket, iByteArray, static_cast<int>(iSize), PLATFORM_VALUE(0, MSG_NOSIGNAL));
      if (isError(iResult)) {
        LOG_ERROR("Send failed with error : " << shd::network::tcp::error::get());
        close();
        return false;
      }
      return true;
  ...
  public:
    ...
    void init() {
      ...
      if (!isValid()) {
        LOG_ERROR("Failed to create socket " << errno << " : " << shd::network::tcp::error::get());
      }
    }
    ...
    void setAddr(const std::string_view iAddrString) {
      ...
      if (isError(aResult)) {
        LOG_ERROR("Failed call inet_pton : " << shd::network::tcp::error::get());
        close();
        return;
      }
      else if (aResult == 0) {
    ...
    bool bind() {
      ...
      if (isError(::bind(_socket, (SOCKTYPE*)&_sin, sizeof(_sin)))) {
        LOG_ERROR("Failed to bind socket : " << shd::network::tcp::error::get());
        close();
        return false;
      }
      return true;
    ...
    bool listen(int iNbMaxConnection) {
      ...
      if (isError(::listen(_socket, iNbMaxConnection))) {
        LOG_ERROR("Failed to listen socket : " << shd::network::tcp::error::get());
        close();
        return false;
      }
      return true;
    ...
    bool connect() {
      ...
      if (isError(connectStatus)) { //it returns 0 if no error occurs
        LOG_ERROR("Failed to connect : " << shd::network::tcp::error::get());
        close();
        return false;
      }
      return true;
    ...
    bool writeError(const shd::network::message::Header::id iType) {
      shd::network::message::error::text aText;
      auto [aMessage, aMessageSize] = shd::network::message::error::create(iType, aText);
      return write(aMessage, aMessageSize);
    }

    bool write(shd::network::buffer::Output& iBuffer) {
      return write(iBuffer, sizeof(shd::network::message::Header) + iBuffer.getHeader()->getPayloadSize());
    }

    bool write(const shd::network::buffer::Output& iBuffer, const size_t iSize) {
      RETURN_IF_SOCKET_INVALID;
    ...
    std::pair<bool, int> read(const char* iCallerInfo, char* oResult, const size_t iReadSize)
    {
      ...
      LOG_MESSAGE("[Socket] Read : " << static_cast<int>(((shd::network::message::Header*)oResult)->code) << " -- Payload size : " << ((shd::network::message::Header*)oResult)->payloadSize);
      if (isError(aNbCharRead)) {
        shd::network::tcp::error::error_t aError = network::tcp::error::get();
        if (aError.first == misc::platform::varWinLin(10035, 11)) {
          writeError(shd::network::message::Header::id::HeaderOnly);
          LOG_ERROR(iCallerInfo << "Header only message");
        } else if (aError.first == misc::platform::varWinLin(10, 104)) {
          LOG_ERROR(iCallerInfo << " -- " << shd::network::tcp::error::get());
        }
        else {
          LOG_ERROR(iCallerInfo << "Failed to recv : " << shd::network::tcp::error::get());
          writeError(shd::network::message::Header::id::RecvFailed);
        }
        close(getLogPrefix());
        ...
        writeError(shd::network::message::Header::id::InvalidRead);
        close();
    ...
    bool read(const char* iCallerInfo, network::buffer::Input &ioBuffer)
    {
      ioBuffer.reset();
      {
        auto [ aSuccess, aSize ] = read(iCallerInfo, ioBuffer.getData(), sizeof(::shd::network::message::Header));
        if (!aSuccess || aSize != sizeof(::shd::network::message::Header)) {
          return false;
        }
      }
    ...
    std::pair<bool, int> readString(const size_t iOutputStringSize, shd::network::buffer::Input& aBuffer) {
